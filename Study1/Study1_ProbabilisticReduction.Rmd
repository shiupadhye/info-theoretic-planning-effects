---
title: "Duration Analysis (Last modified: 09/06/25)"
output:
...
durCoefData$Predictor <- factor(durCoefData$Predictor, levels = c("Unigram Predictability", "Forward Predictability","Backward Predictability", "DeltaBwPred", "conditional PMI")) 
```
...
durCoefData.filtered <- durCoefData %>%
  ...
  #filter(Model != "Model with conditional PMI") %>%
  filter(!(Model == "Model with Backward Predictability" & Predictor %in% c("conditional PMI", "DeltaBwPred"))) %>%
  filter(!(Model == "Model with DeltaBwPred" & Predictor %in% c("Backward Predictability", "conditional PMI"))) %>%
  filter(!(Model == "Model with conditional PMI" & Predictor %in% c("Backward Predictability", "DeltaBwPred")))
```

```{r}
durCoefData.filtered$Model <- factor(durCoefData.filtered$Model, levels = c("Model with Backward Predictability","Model with DeltaBwPred","Model with conditional PMI")) 
```


```{r}
dur.colors <- c('#3E8D31','#295C9D','#7E629E')
...
df <- LRT.results %>%
  ...
  #filter(Model != 'conditional PMI') %>%
  mutate(
    Model = factor(Model, levels = c(
      "Backward Predictability",
      "conditional PMI",
      "DeltaBwPred",
      "DeltaBwPred + conditional PMI"
    )),
...
comparisons <- list(
  c("Backward Predictability", "DeltaBwPred"),
  c("DeltaBwPred", "conditional PMI"),
  c("DeltaBwPred", "DeltaBwPred + conditional PMI"),
  c("conditional PMI", "DeltaBwPred + conditional PMI")
)
...
plt.modelComp <- ggplot(df, aes(x = Model, y = DeltaLogLik)) +
  ...
  geom_signif(
   ...
   scale_x_discrete(labels = c(
    "Backward Predictability" = expression(bold("Backwaqrd Predictability")),
    "conditional PMI" = expression(bold("conditional PMI")),
    "DeltaBwPred" = expression(Delta * bold("BwPred")),
    "DeltaBwPred + conditional PMI" = expression(Delta * bold("BwPred + conditional PMI"))
  )) +
  theme_classic() +
  theme(
...
```{r}
df.bwOnly <- LRT.results %>%
  filter(Model != 'Baseline') %>%
  filter(Model != 'conditional PMI') %>%
  filter(Model != 'DeltaBwPred + conditional PMI') %>%
  mutate(
    Model = factor(Model, levels = c(
      "Backward Predictability",
      "DeltaBwPred"
...
# Plot
plt.modelComp.backwardOnly <- ggplot(df.bwOnly, aes(x = Model, y = DeltaLogLik)) +
  geom_bar(stat = "identity", fill = '#7E629E', width = 0.7) +
  ylab(expression(Delta * bold("LogLik"))) +
  xlab("Predictor added to baseline model") +
  ylim(0,5500) +
  ...
  geom_signif(
   ...
   scale_x_discrete(labels = c(
    "Backward Predictability" = expression(bold("Backward Predictability")),
    "DeltaBwPred" = expression(Delta * bold("BwPred"))
  )) +
  theme_classic() +
  theme(
...
```{r}
plt.modelComp.backwardOnly
```

```{r}
plt.combined <- plt.modelCoeffs + plt.modelComp + plot_layout(widths = c(2, 1))
ggsave("plots/Study1_ModelCoeffs&LRT_updated.png",plt.combined , width = 12, height = 5, dpi = 300)
```
