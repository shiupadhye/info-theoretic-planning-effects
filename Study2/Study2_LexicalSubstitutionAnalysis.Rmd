...
```{r}
scores <- read_csv('data/SWBD_LexSubsAnalysisData.csv')
```
...
```{r}
lexSubsAnnotated <- read_csv('data/SWBD_lexicalSubstitutions_samePOS_categorized.csv')
```
...
```{r}
save_model_coeffs(models, "results/Study2_ModelCoefficients_allCategories.csv")
```
...
```{r}
lexSubsModelCoef$Predictor <- factor(lexSubsModelCoef$Predictor, levels = c("Unigram Predictability", "Forward Predictability","Semantic Distance","Phonetic Distance","Delta Backward Predictability", "condPMI")) 
...
lexSubsModelCoef.filtered <- lexSubsModelCoef %>%
  ...
  filter(!(model == "Model with Delta Backward Predictability" & Predictor %in% c("condPMI"))) %>%
  filter(!(model == "Model with condPMI" & Predictor %in% c("Delta Backward Predictability")))
```

```{r}
lexSubsModelCoef.filtered$model <- factor(lexSubsModelCoef.filtered$model, levels = c("Model with Delta Backward Predictability", "Model with condPMI")) 
```
...
```{r}
saveRDS(plt.modelCoeffs.all,"plots/rds/Study2_ModelCoefficients_allCategories.rds")
```
...
```{r}
LRT.results <- read_csv('results/LRT-results.csv')
head(LRT.results)
```
...
ypos <- top + c(1.0, 2.0, 3.8) * step   # ← tweak these if needed
...
```{r}
saveRDS(plt.LRT,"plots/rds/Study2_LRT.rds")
```
...
```{r}
save_model_coeffs(sem.models, "results/Study2_ModelCoefficiens_sem.csv")
```
...
```{r}
save_model_coeffs(phon.models, "results/Study2_ModelCoefficiens_phon.csv")
```
...
```{r}
save_model_coeffs(ms.models, "results/Study2_ModelCoefficiens_ms.csv")
```
...
```{r}
save_model_coeffs(mixed.models, "results/Study2_ModelCoefficiens_mixed.csv")
```
...
```
```{r}
lexSubsModelCoef.sem$Predictor <- factor(lexSubsModelCoef.sem$Predictor, levels = c("Unigram Predictability", "Forward Predictability","Semantic Distance","Phonetic Distance","Delta Backward Predictability", "condPMI")) 
...
```{r}
lexSubsModelCoef.sem.filtered$Model <- factor(lexSubsModelCoef.sem.filtered$Model, levels = c("Model with Delta Backward Predictability", "Model with condPMI")) 
```
...
```{r}
lexSubsModelCoef.phon$Predictor <- factor(lexSubsModelCoef.phon$Predictor, levels = c("Unigram Predictability", "Forward Predictability","Semantic Distance","Phonetic Distance","Delta Backward Predictability", "condPMI")) 
...
```{r}
lexSubsModelCoef.phon.filtered$Model <- factor(lexSubsModelCoef.phon.filtered$Model, levels = c("Model with Delta Backward Predictability", "Model with condPMI")) 
```
...
```{r}
saveRDS(plt.modelCoeffs.phon,"plots/rds/Study2_ModelCoefficients_phon.rds")
```

### morphosyntactic
```{r}
lexSubsModelCoef.ms <- read_csv('results/Study2_ModelCoefficiens_ms.csv')
...
```{r}
lexSubsModelCoef.ms$Predictor <- factor(lexSubsModelCoef.ms$Predictor, levels = c("Unigram Predictability", "Forward Predictability","Semantic Distance","Phonetic Distance","Delta Backward Predictability", "condPMI")) 
...
```{r}
lexSubsModelCoef.ms.filtered$Model <- factor(lexSubsModelCoef.ms.filtered$Model, levels = c("Model with Delta Backward Predictability", "Model with condPMI")) 
```
...
```{r}
saveRDS(plt.modelCoeffs.ms,"plots/rds/Study2_ModelCoefficients_ms.rds")
```


```{r}
lexSubsModelCoef.mixed <- read_csv('results/Study2_ModelCoefficiens_mixed.csv')
...
```{r}
lexSubsModelCoef.mixed$Predictor <- factor(lexSubsModelCoef.mixed$Predictor, levels = c("Unigram Predictability", "Forward Predictability","Semantic Distance","Phonetic Distance","Delta Backward Predictability", "condPMI")) 
...
```{r}
lexSubsModelCoef.mixed.filtered$Model <- factor(lexSubsModelCoef.mixed.filtered$Model, levels = c("Model with Delta Backward Predictability", "Model with condPMI")) 
```
...
```{r}
saveRDS(plt.modelCoeffs.mixed,"plots/rds/Study2_ModelCoefficients_mixed.rds")
```
